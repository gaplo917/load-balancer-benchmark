...
docker run -ti --rm --network=host --ulimit nofile=65535:65535 alpine/bombardier --http2 -o json -p result -c $warmup_concurrency -n 50000 -t 60s -l $1 | jq '.' | tee $warmup_result > /dev/null;
...
do
  ...
  docker run -ti --rm --network=host --ulimit nofile=65535:65535 alpine/bombardier --http2 -o json -p result -c $value -n 1000000 -t 60s -l $1 | jq '.' | tee $json_result > /dev/null;
